...
namespace defarge
{
    public class BaseTest
    {
        ...
        protected static readonly string baseUrl = "http://localhost:5209/api";
...
private static Dictionary<string, long> _lastIdMap = null;
        ...
        public static long getLastId( string domainObj)
        {
            ...
            protected static Stack<Query> lastQuery = new Stack<Query>();
            protected static Dictionary<long, Query> mapQuery = new Dictionary<long, Query>();
            ...
            protected static Stack<PrincipalPassword> lastPrincipalPassword = new Stack<PrincipalPassword>();
            protected static Dictionary<long, PrincipalPassword> mapPrincipalPassword = new Dictionary<long, PrincipalPassword>();
        ...
        protected static async Task PutAsync<T>(string endpoint, long id, T obj)
        {
            var json = JsonSerializer.Serialize(obj);
            var content = new StringContent(json, Encoding.UTF8, "application/json");
            var response = await httpClient.PutAsync($"api/{endpoint}/{id}", content);
            response.EnsureSuccessStatusCode();
        }

        protected static async Task DeleteAsync(string endpoint, long id)
        {
            var response = await httpClient.DeleteAsync($"api/{endpoint}/{id}");
            response.EnsureSuccessStatusCode();
        }
    }
}
